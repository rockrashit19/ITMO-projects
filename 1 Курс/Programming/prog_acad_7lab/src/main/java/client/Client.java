...
public class Client {
    ...
    public boolean authenticate() {
        try {
            ...
            String choice;
            while (true) {
                choice = inputManager.getStringInput("Enter choice (1 or 2): ", false).trim();
                if (choice.equals("1") || choice.equals("2")) break;
                outputManager.println("Please enter 1 (Login) or 2 (Register).");
            }

            username = inputManager.getStringInput("Enter username: ", false).trim();
            password = inputManager.getStringInput("Enter password: ", false);

            String cmd = choice.equals("1") ? "login" : "register";
            CommandRequest request = new CommandRequest(cmd, "", null, username, password);
