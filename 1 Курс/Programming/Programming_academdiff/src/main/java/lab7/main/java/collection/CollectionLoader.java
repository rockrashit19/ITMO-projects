...
import lab7.main.java.exception.*;
import lab7.main.java.util.OutputManager;
import lab7.main.java.util.ZonedDateTimeAdapter;
...
public class CollectionLoader {
    ...
    public List<LabWork> loadCollection() throws LoadingFileException {
        outputManager.println("Loading collection from file: " + filePath);

        try {
            if (filePath == null || filePath.isEmpty()) {
                throw new LoadingFileException("File path is not set. Please set the environment variable.");
            }
            ...
            if (!Files.isReadable(Paths.get(filePath))) {
                throw new LoadingFileException("Cannot read file: " + filePath + ". Check file permissions.");
            }
            ...
            throw new LoadingFileException("Error parsing JSON syntax in file " + filePath + ": " + e.getMessage());
        } catch (java.io.IOException e) {
            throw new LoadingFileException("Error reading or accessing file " + filePath + ": " + e.getMessage());
        } catch (Exception e) {
            outputManager.println("An unexpected error occurred during file loading: " + e.getMessage());
            throw new LoadingFileException("An unexpected error occurred during file loading: " + e.getMessage());
        }
    }
}